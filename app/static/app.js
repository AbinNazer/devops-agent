(() => {
  ...
  let conversationId, socket, terminal, fitAddon;
  ...
  async function init() {
    const [conv, providers] = await Promise.all([fetch('/api/conversations', {method:'POST'}), fetch('/api/providers')]);
    conversationId = (await conv.json()).id; const data = await providers.json();
  ...
  function openTerminal() { window.open('/terminal', '_blank', 'noopener'); }
  function fitTerminal() { if (!fitAddon || !terminal) return; fitAddon.fit(); if (socket?.readyState === WebSocket.OPEN) socket.send(JSON.stringify({type:'resize',rows:terminal.rows,cols:terminal.cols})); }
  ...
  $('terminal-button').onclick = openTerminal; $('welcome-terminal').onclick = openTerminal; $('close-terminal').onclick = () => { $('terminal-panel').hidden = true; if (socket) socket.close(); socket = null; };
  ...
  if (window.visualViewport) window.visualViewport.addEventListener('resize', () => { if (!$('terminal-panel').hidden) fitTerminal(); });
